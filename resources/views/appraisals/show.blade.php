                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                        <div>
                            ...
                            <p class="text-lg font-medium text-gray-900">{{ $appraisal->employee?->name }} ({{ $appraisal->employee?->nip }})</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Penilai:</p>
                            {{-- Gunakan null-safe operator (?->) --}}
                            <p class="text-lg font-medium text-gray-900">{{ $appraisal->appraiser?->name }}</p>
                        </div>
                        <div>
                    ...
                    @forelse ($appraisal->criterionScores as $score)
                        <div class="border p-4 rounded-lg mb-4 bg-gray-50">
                            <h4 class="font-medium text-md text-gray-700">{{ $loop->iteration }}. {{ $score->criterion?->name }}</h4> {{-- Gunakan null-safe operator --}}
                            <p class="text-sm text-gray-600 mb-2">{{ $score->criterion?->description }}</p> {{-- Gunakan null-safe operator --}}
                            <p class="text-md font-semibold text-indigo-700">Skor: {{ $score->score }} / 5</p>
