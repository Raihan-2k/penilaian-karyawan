        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <h3 class="font-semibold text-lg text-gray-800 mb-4">Submission Tugas</h3>
                    @forelse ($task->submissions as $submission)
                        <div class="border p-4 rounded-lg mb-4 bg-gray-50">
                            <p class="text-sm text-gray-600">Dikumpulkan Oleh: {{ $submission->submittedBy->name }}</p>
                            <p class="text-sm text-gray-600">Tanggal Submit: {{ $submission->submission_date->format('d M Y H:i') }}</p>
                    ...
                    @empty
                        <p class="text-gray-500">Belum ada submission untuk tugas ini.</p>
                    @endforelse
                    ...
                    <div class="mt-6 flex justify-start">
                        ...
                        <form action="{{ route('tasks.destroy', $task) }}" method="POST" class="inline-block ml-4">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-700 active:bg-red-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150" onclick="return confirm('Apakah Anda yakin ingin menghapus tugas ini?')">
                                Hapus Tugas
                            </button>
                        </form>
                    </div>
